...
static void max_size(void) {
	...
	ft::Vector<int> v1;
	std::vector<int> v2;
    check("v1.max_size() == v2.max_size() empty", v1.max_size(), v2.max_size());
	v1.push_back(10);
	v2.push_back(10);
	check("v1.max_size() == v2.max_size() fill", v1.max_size(), v2.max_size());
}
...
static void access_operator(void) {
	...
	try {
		...
		std::cout << e.what() << ": " << std::string(7, ' ') << BOLD << GREEN << SUCCESS << RESET << std::endl;
	}
	try {
		std::cout << "v2.at(100): " << v2.at(100) << ": " << FAIL  << std::endl;
	} catch (std::exception &e) {
		std::cout << e.what() << ": " << std::string(20, ' ') << BOLD << GREEN << SUCCESS << RESET << std::endl;
	}
	try {
		std::cout << "v1.at(-1): " << v1.at(-1) << ": " << FAIL  << std::endl;
	} catch (std::exception &e) {
		std::cout << e.what() << ": " << std::string(8, ' ') << BOLD << GREEN << SUCCESS << RESET << std::endl;
	}
	try {
		std::cout << "v2.at(-1): " << v2.at(-1) << ": " << FAIL  << std::endl;
	} catch (std::exception &e) {
		std::cout << e.what() << ": " << std::string(21, ' ') << BOLD << GREEN << SUCCESS << RESET << std::endl;
	}
}
