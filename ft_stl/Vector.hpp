...
namespace ft {
	...
	class Vector {
		...
		public:
			...
			Vector(InputIterator begin, InputIterator end, const allocator_type &allocator = allocator_type()) : _container_length(0), _container_size(0), _container(0), _allocator(allocator) {
				_container = _allocator.allocate(0);
				assign(begin, end);
			};
			Vector(size_type n, const_reference value = value_type(), const allocator_type &allocator = allocator_type()) : _container_length(0), _container_size(0), _container(0), _allocator(allocator){
				_container = _allocator.allocate(0);
				assign(n, value);
			};
			Vector(const Vector &other) : _container_length(0), _container_size(0), _container(0), _allocator(other._allocator) {
			    *this = other;
			};

			~Vector(void) {_allocator.deallocate(_container, _container_size);};
			...
			void pop_back(void) {if (_container_length) _container_length--;};
