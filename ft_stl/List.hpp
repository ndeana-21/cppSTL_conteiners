...
namespace ft {
	...
	class List {
		...
		public:
			...
			const_reverse_iterator rend(void) const {return (const_reverse_iterator(_list_begin));};
			...
			void unique(void) {unique(_Equal<value_type>());};
			...
			void merge(List &x) {merge(x, _Smaller<value_type>());};
			...
			void sort(void) {sort(_Smaller<value_type>());};
	...
	template<typename T>
	bool operator<=(List<T> const &lhs, List<T> const &rhs) {return (!(rhs < lhs));};
